...
function IncomePages() {
  ...
  const loadAllData = useCallback(async () => {
    try {
      // 1. Fetch Laporan (Tetap sama)
      const resLaporan = await IncomeServices.getDataLaporan();
      setLaporan(resLaporan);

      // 2. Fetch Histori (Sekarang kirim parameter page 0 dan size 1)
      // Karena kamu cuma butuh "Latest Transaction", ambil 1 data aja cukup
      const resHistori = await IncomeServices.getAllData(0, 1);

      // Cek apakah ada data di dalam .content
      if (resHistori && resHistori.content && resHistori.content.length > 0) {
        // Ambil data terbaru (indeks 0 dari content)
        setLatestTrans(resHistori.content[0]);
      }
    } catch (err) {
        ...
        <div className="bg-white rounded-[8px] p-[16px] shadow-[0_4px_6px_-1px_rgba(0,0,0,0.1)] flex flex-col gap-[10px]">
          ...
          <div className="flex justify-between items-center text-[16px]">
            ...
            <span className="font-medium text-[#16A34A]">
              Rp {laporan.totalIncome?.toLocaleString("id-ID")}
            </span>
          </div>
          ...
          <div className="flex justify-between items-center text-[16px]">
            ...
            <span className="font-medium text-[#DC2626]">
              Rp {laporan.totalExpenses?.toLocaleString("id-ID")}
            </span>
          </div>
          ...
          <div className="flex justify-between items-center text-[16px] font-bold border-t pt-2">
            ...
            <span className="text-[#16A34A]">
              Rp {laporan.profit?.toLocaleString("id-ID")}
            </span>
          </div>
        ...
        <div className="bg-white rounded-[8px] p-[16px] shadow-[0_4px_6px_-1px_rgba(0,0,0,0.1)] flex flex-col gap-[10px]">
          ...
          <div className="flex justify-between items-center text-[14px]">
            ...
            <span className="text-[#DC2626] font-medium">
              Rp {latestTrans.buyPrice?.toLocaleString("id-ID")}
            </span>
          </div>
          ...
          <div className="flex justify-between items-center text-[14px]">
            ...
            <span className="text-[#16A34A] font-medium">
              Rp {latestTrans.sellPrice?.toLocaleString("id-ID")}
            </span>
          </div>
          ...
          <div className="flex justify-between items-center text-[14px]">
            <span className="text-[#6B7280]">Pengeluaran</span>
            <span className="text-[#DC2626] font-medium">
              Rp {latestTrans.expensesAmount?.toLocaleString("id-ID") || 0}
            </span>
          </div>
